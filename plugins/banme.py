...
from config import BanMeReplayAddress, BOT_NAME, AllGroupMemberDatabaseName
from others.operational_database import find_data
...
def unban(client, message):
    try:
        ...
        identity = asyncio.run(find_data(AllGroupMemberDatabaseName, message.chat.id, filter_find))['Identity']
        if identity == "restricted":
            ...
            send_message = message.reply_text("❗**用户并未被封禁**")
