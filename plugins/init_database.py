...
from others.operational_database import write_data, check_database, check_table, update_data_one
        ...
        if user_check.status in ('administrator', 'creator'):
            ...
            if check_database(message.chat.id) and check_table(message.chat.id, AllGroupMemberDatabaseName) is False:
                for members in iter_chat:
                    ...
                    update_data_one(AllGroupMemberDatabaseName, message.chat.id, update_data, filter_dict)
                update_end = time()
                send_message.edit("**更新完成, 耗时{0:.2f}秒**".format(update_end - update_start))
            else:
